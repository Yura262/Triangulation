...
Click to add points, see numbered vertices and triangles
"""
...
class DelaunayGUI:
    def __init__(self):
        ...
        plt.subplots_adjust(left=0.1, bottom=0.2, right=0.85, top=0.95)
    ...
    def setup_controls(self):
        ...
        ax_random = plt.axes([0.32, 0.08, 0.1, 0.04])
        ...
        # Grid points button
        ax_grid = plt.axes([0.43, 0.08, 0.1, 0.04])
        self.btn_grid = Button(ax_grid, 'Add Grid', color='lightyellow', hovercolor='yellow')
        self.btn_grid.on_clicked(self.add_grid_points)
        
        # Grid points button
        ax_grid = plt.axes([0.54, 0.08, 0.1, 0.04])
        self.btn_grid = Button(ax_grid, 'Update Plot', color='lightblue', hovercolor='blue')
        self.btn_grid.on_clicked(self.update_plot)
        
        # Checkbox for display options
        ax_check = plt.axes([0.86, 0.3, 0.13, 0.3])
        labels = ['Vertices', 'Vertex Numbers', 'Triangles', 'Triangle Numbers', 'Edges', 'Circumcircles']
        visibility = [self.show_vertices, self.show_vertex_numbers, self.show_triangles, 
                     self.show_triangle_numbers, self.show_edges, self.show_circumcircles]
    ...
    def compute_triangulation(self):
        ...
        try:
            # Create points for C++
            cpp_points = [triangulation.Point(int(x), int(y)) for x, y in self.points]
            ...
            # Convert triangles to Python format
            self.triangles = []
            for tri in cpp_triangles:
                # Find indices of vertices in our points list
                indices = []
...
def main():
    ...
    print("- Use the checkboxes to toggle display options")
